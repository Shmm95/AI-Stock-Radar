...
using a throwaway state file so the real `data/live/position_state.json`
is never touched.
...
script deletes its own throwaway state/log files when done, whatever
the outcome.
"""
...
def main() -> int:
    ...
    save_position_state(seeded_state, SMOKE_STATE_PATH)
    print(f"Seeded a pending BUY for {SMOKE_TICKER} in throwaway state: {SMOKE_STATE_PATH}")

    print("\n=== RUN 1: entry ===")
    result_1 = runner.run_daily_decision(
        state_path=SMOKE_STATE_PATH,
        decision_log_directory=SMOKE_LOG_DIRECTORY,
        enable_crypto_orders=True,
    )
    decision_1 = result_1["decision"]
    ...
    state_after_run_1 = runner.load_position_state(SMOKE_STATE_PATH)
    ...
    save_position_state(state_after_run_1, SMOKE_STATE_PATH)
    print(f"\nSeeded a pending signal-based EXIT for {SMOKE_TICKER}.")

    print("\n=== RUN 2: signal-based exit ===")
    result_2 = runner.run_daily_decision(
        state_path=SMOKE_STATE_PATH,
        decision_log_directory=SMOKE_LOG_DIRECTORY,
        enable_crypto_orders=True,
    )
    decision_2 = result_2["decision"]
    ...
    _cleanup()
    print(f"\nThrowaway state/log files removed. Real state at "
