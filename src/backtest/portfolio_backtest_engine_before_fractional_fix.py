...
def _attempt_open_position(
    ...
    *,
    ...
    while quantity > 0 and position_value + entry_fee > state.cash:
        if _allow_fractional(ticker, config):
            affordable = max(state.cash - config.minimum_fee, 0.0) / entry_price
            quantity = _normalize_quantity(affordable, True)
        else:
            quantity = max(quantity - 1.0, 0.0)
