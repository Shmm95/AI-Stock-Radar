...
def run_matched_benchmark(
    ...
    *,
    ...
    for ticker in symbols:
        ...
        if quantity > 0 and value + entry_fee > cash:
            commission_multiplier = 1.0 + max(commission, 0.0)
            affordable_after_minimum_fee = (
                max(cash - minimum_fee, 0.0) / entry
            )
            affordable_after_commission = (
                max(cash, 0.0) / (entry * commission_multiplier)
            )
            affordable = min(
                quantity,
                affordable_after_minimum_fee,
            ...
            if fractional:
                ...
                quantity = (
                    floor(max(affordable, 0.0) / quantity_step)
                    * quantity_step
                )
                quantity = round(quantity, 6)
            else:
                quantity_step = 1.0
                quantity = float(max(floor(affordable), 0))

            for _ in range(8):
                value = quantity * entry
                entry_fee = fee(value)
                if value + entry_fee <= cash + 1e-9:
                    break

                excess = value + entry_fee - cash
                reduction_steps = max(
                    1,
                    ceil(excess / (entry * quantity_step)),
                )
                quantity = max(
